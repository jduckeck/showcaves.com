...
// find db entry
//---------------------------------------------------------------------------------------------------
$sql = "SELECT CaveName, Titel, Beschreibung, Photographer, PhotographerLink, Copyright, CopyrightLink FROM myImages WHERE Filename ='".$pic."'";
...
if($result) {
    ...
    if ( $row ) {
        ...
        $Title    = myUmlaute($row->Titel);
        $break = false;
        if (0 < strlen($row->Beschreibung)) {
            $Caption = "<b>Bild:</b> ".myUmlaute($row->Beschreibung);
            $break = true;
        }
        if ( $break ) {
            $Caption .= "<br>";
        ...
        if (0 < strlen($row->Photographer)) {
            $Caption .= "<b>Fotograf:</b> ".$PhotographerCaption;
            $break = true;
        }
        if ( $break ) {
            $Caption .= "<br>";
...
<h2 align="center"><?php echo $CaveName ?></h2>
<h1 align="center"><?php echo $Title ?></h1>



<table align="center" border="0" cellspacing="0" cellpadding="0" width="<?php echo $size[0] ?>">
<tr><td><img src="<?php echo $relRoot ?>image.php?pic=<?php echo $pic ?>&type=i&size=big&key=<?php echo time() ?>" alternate="<?php echo $CaveName ?> - <?php echo $Title ?>" width="<?php echo $size[0] ?>" height="<?php echo $size[1] ?>"></td></tr>
<tr><td class="imgCaption"><?php echo $Caption ?></td></tr>
</table>

<h1 align="center"><a href="<?php echo $HTTP_REFERER ?>">Zurück</a></h1>
...
<a target="_top" href="<?php echo $relRoot ?>german/index.html">Hauptseite</a>
</td></tr>
...
if(lastmoddate == 0){               // unknown date (or January 1, 1970 GMT)
   document.writeln("unbekannt,")
   } else {
   document.writeln(lastmod + ",")
}
// End -->
</script>
<a href="<?php echo $relRoot ?>german/Impressum.html">Impressum</a>,
&copy;&nbsp;<a href="<?php echo $relRoot ?>german/Jochen.html">Jochen Duckeck</a>.
</td></tr>
</table>
